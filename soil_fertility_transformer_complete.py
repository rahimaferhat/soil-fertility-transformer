...
def get_data():
    ...
    try:
        ...
        if os.path.exists(x_path) and os.path.exists(y_path):
            print(f"📂 Loading real data from {DRIVE_PATH}...")
            X = np.load(x_path)
            Y = np.load(y_path)
        ...
        print(f"⚠️ {e}")
        print("⚠️ Falling back to synthetic data creation.")
...
print("... 🧐 Verification: Attempting to reload model immediately...")
try:
    ...
    if diff < 1e-6:
        print("✅ SUCCESS: Model saved and reloaded perfectly! Your file is safe.")
    else:
        print("⚠️ WARNING: Predictions differ slightly (check precision).")
        
except Exception as e:
    print(f"❌ ERROR: Failed to reload the model.\n{e}")

print("\n🎉 DONE! You can now download your model from:", model_path)
